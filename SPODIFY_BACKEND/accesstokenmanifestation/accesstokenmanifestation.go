...
func saveLocalToken(token string, expiry time.Time) {
	...
	tokenData := TokenData{   // create TokenData
		AccessToken: token,
		Expiry:      expiry,
	}
...
func ManifestAccessToken() TokenData {
	...
	if err != nil || time.Now().After(tokenData.Expiry) { // if an error occured or the token has expired
		...
		loadDotEnv()
		clientID, clientSecret := loadLocalCredentials()
		token, expiry := loadRemoteToken(clientID, clientSecret)
		tokenData.AccessToken = token
		tokenData.Expiry = expiry
		saveLocalToken(token, expiry) // rewrite the access token
	}
	return tokenData
}
