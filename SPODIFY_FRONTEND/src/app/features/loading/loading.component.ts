...
import {PCFSoftShadowMap, Texture, Scene, PerspectiveCamera, WebGLRenderer, SphereGeometry, MeshPhongMaterial, Mesh, TextureLoader,  Vector3, Euler, BoxGeometry, DirectionalLight, AmbientLight} from 'three';
...
export class LoadingComponent {
  ...
  ngAfterViewInit() {
    ...
    const camera: PerspectiveCamera = new PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.5, 1000);
    ...
    const material: MeshPhongMaterial = new MeshPhongMaterial({ color: 0x1DB954, specular: 0x696969 }); // create sphere material to be green responsive to light
    const sphere: Mesh = new Mesh(geometry, material); // create sphere
    sphere.castShadow = true;
    sphere.receiveShadow = true;
    scene.add(sphere); // add sphere to scene
    const spotifySignalWaves:Texture = new TextureLoader().load('/SpotifySignalWaves.png'); // load the signal waves
    const decalMaterial:MeshPhongMaterial = new MeshPhongMaterial({ map: spotifySignalWaves, transparent: true, specular: 0x696969 }); // material for transparent png
    const decalPosition:Vector3 = new Vector3(0.8, 0.9, 1); // near the surface
    const decalOrientation:Euler = new Euler(Math.PI / -4, Math.PI / 6, Math.PI / 6); // position facing the user
    const decalSize:Vector3 = new Vector3(1.7, 1.7, 1.7); // self explanatory
    const decalGeometry:DecalGeometry = new DecalGeometry(sphere, decalPosition, decalOrientation, decalSize); // create decal geometry
    ...
    const plateMaterial:MeshPhongMaterial = new MeshPhongMaterial({ color: 0x696969 }); // fifty shades of grey lol
    ...
    scene.add(plate);
    const directionalLight:DirectionalLight = new DirectionalLight(0xFFFFFF, 1); // create light
